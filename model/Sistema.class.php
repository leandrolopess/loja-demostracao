...
class Sistema {
  ...
  //    original =  Produto maçã do Amor  - produto-maca-do-amor 
        ...
        return trim(substr($string, 0, 200));
    }
    ...
    static function limpaCPF_CNPJ($valor){
     ...
     $valor = str_replace("produtos", " ", $valor);
     // $valor = str_replace("conteudo", " ", $valor);
     $valor = str_replace([0-9], " ", $valor);
     return $valor;
    }

    static function limpaCep($valor){
     $valor = trim($valor);
     $valor = str_replace(".", " ", $valor);
     $valor = str_replace("-", " ", $valor);
     $valor = str_replace(" ", "", $valor);
    ...
    return preg_replace("/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/", "\$1.\$2.\$3/\$4-\$5", $cnpj_cpf);
     ...
     static function criaResumo($string,$caracteres) { 
    ...
    static function ValorDesconto ( $valor, $desconto=NULL ) {
    $total = $valor; //Valor total sem o desconto
    $pctm = $desconto; // porcentagem do desconto
    $valor_total_com_desconto = $total - ($total / 100 * $pctm); //Valor total com o desconto
    $valor_do_desconto = $total / 100 * $pctm; // Somente o valor do desconto
    return $valor_do_desconto;
    }

    static function ValorTotalDesconto ( $valor, $desconto=NULL ) {
    $total = $valor; //Valor total sem o desconto
    $pctm = $desconto; // porcentagem do desconto
    $valor_total_com_desconto = $total - ($total / 100 * $pctm); //Valor total com o desconto
    $valor_do_desconto = $total / 100 * $pctm; // Somente o valor do desconto
