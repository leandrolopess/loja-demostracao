...
class Pedidos extends Conexao{
    ...
    // Função que faz a gravação do pedido no banco de dados
    function PedidoGravar($cliente, $cod, $ref, $freteValor=null, $freteTipo=null){
        ...
        //Função que grava os itens do pedido
    ...
    function GetPedidosCliente($cliente=null){
     ...
     $query .= $this->PaginacaoLinks("ped_id", $this->prefix. "pedidos WHERE ped_cliente = {$cli}");
     ...
     // Se pedido for cancelado
    function PedidoUpdateStatus($referencia){        
                  
            // montando a SQL
            $query = "UPDATE {$this->prefix}pedidos SET ped_pag_status = 'Cancelado'";
            $query .=" WHERE ped_ref = :ref";
           
            // passando parâmetros                             
            $params = array(':ref'  =>$referencia);
    ...
    // Função que faz a gravação dos itens do pedido no banco de dados
